...
impl TokenManager {
    ...
    pub async fn set_forbidden(&self, account_id: &str, reason: &str) -> Result<(), String> {
        ...
        // Pass reason as both reason AND raw_error_response so Show Raw can display full response
        crate::modules::account::mark_account_forbidden(account_id, reason, None, Some(reason))?;
